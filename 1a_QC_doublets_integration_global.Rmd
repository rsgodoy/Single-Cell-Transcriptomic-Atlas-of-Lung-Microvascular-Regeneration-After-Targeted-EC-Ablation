...
This notebook was run on desktop R v3.6
...
```{r}
seurat <- readRDS("./0_raw_data_David_new_data_Nov_2020/seurat_filtered.rds")
```
...
```{r}
seurat$Doublet <- sce$scDblFinder.score > 0.15 # Find this gives better results than the "nearestClass"
```

```{r}
doubletfig <- DimPlot(seurat, group.by="Doublet")
doubletfig
#ggsave(doubletfig, filename = "/Users/rafaelgodoy/Desktop/Doublets.png", width=12, height=8, dpi = 900)
```
...
ggsave(cyclefig, filename = "/Users/rafaelgodoy/Desktop/CellCycle.png", width=12, height=8, dpi = 900)
cyclefig
```
...
```{r}
#saveRDS(seurat, file = "~/Projects/stewart_mouse/data/seurat_filtered_processed.rds")
```
